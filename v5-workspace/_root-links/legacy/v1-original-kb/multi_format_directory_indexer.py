...
def run_tiered_indexing_on_directory(target_directory, output_prefix="custom_index", output_formats=["json"], output_dir=None):
    ...
    script_base_path = os.path.expanduser("~/avatararts-kb")
    ...
    for fmt in output_formats:
        if fmt != "json":
            convert_json_to_format(phase3_output, output_prefix, fmt, output_path)
...
def convert_json_to_format(json_file, output_prefix, format_type, output_path):
    ...
    if format_type == 'csv':
        csv_file = os.path.join(output_path, f"{output_prefix}_index.csv")
        ...
        md_file = os.path.join(output_path, f"{output_prefix}_index.md")
        ...
        html_file = os.path.join(output_path, f"{output_prefix}_index.html")
...
def main():
    parser = argparse.ArgumentParser(description="Multi-Format Directory Indexer for AVATARARTS Knowledge Base")
    ...
    parser.add_argument("formats", nargs="?", default="json", help="Comma-separated formats (json,csv,md,html)")
    ...
    success = run_tiered_indexing_on_directory(directory, prefix, formats, args.output_dir)
