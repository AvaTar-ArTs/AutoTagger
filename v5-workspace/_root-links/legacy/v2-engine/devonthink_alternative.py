...
def main():
    ...
    if args.action == 'index':
        ...
        indexer_script = os.path.expanduser("~/AutoTagger/v2-engine/multi_format_directory_indexer.py")
        cmd = [sys.executable, indexer_script, args.directory, args.prefix, "json,csv,md"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0:
            # Add DevonThink-like features
            json_file = os.path.expanduser(f"~/AutoTagger/v2-engine/{args.prefix}_phase3.json")
