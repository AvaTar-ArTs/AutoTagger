...
def main():
    ...
    db_path = os.path.expanduser("~/AutoTagger/v3-dev/knowledge_base.db")
    ...
    if args.action == 'index':
        ...
        indexer_script = os.path.expanduser("~/AutoTagger/v3-dev/multi_format_directory_indexer.py")
        cmd = [sys.executable, indexer_script, args.directory, args.prefix, "json,csv,md"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0:
            # Add DevonThink-like features
            json_file = os.path.expanduser(f"~/AutoTagger/v3-dev/{args.prefix}_phase3.json")
